...
#define		GIO_TITLE_FONT							FONT14ARIAL//FONT16ARIAL
#define		GIO_TITLE_COLOR									FONT_MCOLOR_WHITE

#define		GIO_TOGGLE_TEXT_FONT					FONT12ARIAL//FONT16ARIAL
...
#define		GIO_CANCEL_X							iScreenWidthOffset + 320 + 143
...
// Madd
#define		GIO_GAME_SETTINGS_X						GIO_DIF_SETTINGS_X
#define		GIO_GAME_SETTINGS_Y						GIO_GUN_SETTINGS_Y
#define		GIO_GAME_SETTINGS_WIDTH					GIO_DIF_SETTINGS_WIDTH

#define		GIO_GUN_SETTINGS_X								GIO_BR_SETTING_X
...
//checkbox to toggle BR selection
...
BOOLEAN		EnterGIOScreen()
{
  VOBJECT_DESC    VObjectDesc;
	...
	if( gGameOptions.ubDifficultyLevel == DIF_LEVEL_EASY )
		...
		ButtonList[ guiDifficultySettingsToggles[ GIO_DIFF_INSANE ] ]->uiFlags |= BUTTON_CLICKED_ON;
...
//		ButtonList[ guiGameStyleToggles[ GIO_PLATINUM ] ]->uiFlags |= BUTTON_CLICKED_ON;		
	...
	{
		case BR_GOOD:
			ButtonList[ guiBROptionToggles[ GIO_BR_GOOD ] ]->uiFlags |= BUTTON_CLICKED_ON;
			break;
		case BR_GREAT:
			ButtonList[ guiBROptionToggles[ GIO_BR_GREAT ] ]->uiFlags |= BUTTON_CLICKED_ON;
			break;
		case BR_EXCELLENT:
			ButtonList[ guiBROptionToggles[ GIO_BR_EXCELLENT ] ]->uiFlags |= BUTTON_CLICKED_ON;
			break;
		case BR_AWESOME:
...
BOOLEAN		RenderGIOScreen()
{
	...
	usPosY += GIO_GAP_BN_SETTINGS-5;
	//DrawTextToScreen( gzGIOScreenText[ GIO_HARD_TEXT ], (UINT16)(GIO_DIF_SETTINGS_X+GIO_OFFSET_TO_TEXT), usPosY, GIO_MAIN_TITLE_WIDTH, GIO_TOGGLE_TEXT_FONT, GIO_TOGGLE_TEXT_COLOR, FONT_MCOLOR_BLACK, FALSE, LEFT_JUSTIFIED );	
	DisplayWrappedString( (UINT16)(GIO_DIF_SETTINGS_X+GIO_OFFSET_TO_TEXT), usPosY, GIO_DIF_SETTINGS_WIDTH, 2, GIO_TOGGLE_TEXT_FONT, GIO_TOGGLE_TEXT_COLOR, gzGIOScreenText[ GIO_INSANE_TEXT ], FONT_MCOLOR_BLACK, FALSE, LEFT_JUSTIFIED );
...
//Madd
	...
	usPosY = GIO_BR_SETTING_Y+2;

	DisplayWrappedString( (UINT16)(GIO_BR_SETTING_X+GIO_OFFSET_TO_TEXT), usPosY, GIO_DIF_SETTINGS_WIDTH, 2, GIO_TOGGLE_TEXT_FONT, GIO_TOGGLE_TEXT_COLOR, gzGIOScreenText[ GIO_BR_GOOD_TEXT ], FONT_MCOLOR_BLACK, FALSE, LEFT_JUSTIFIED );
	usPosY += GIO_GAP_BN_SETTINGS-5;

	DisplayWrappedString( (UINT16)(GIO_BR_SETTING_X+GIO_OFFSET_TO_TEXT), usPosY, GIO_DIF_SETTINGS_WIDTH, 2, GIO_TOGGLE_TEXT_FONT, GIO_TOGGLE_TEXT_COLOR, gzGIOScreenText[ GIO_BR_GREAT_TEXT ], FONT_MCOLOR_BLACK, FALSE, LEFT_JUSTIFIED );
	usPosY += GIO_GAP_BN_SETTINGS-5;

	DisplayWrappedString( (UINT16)(GIO_BR_SETTING_X+GIO_OFFSET_TO_TEXT), usPosY, GIO_DIF_SETTINGS_WIDTH, 2, GIO_TOGGLE_TEXT_FONT, GIO_TOGGLE_TEXT_COLOR, gzGIOScreenText[ GIO_BR_EXCELLENT_TEXT ], FONT_MCOLOR_BLACK, FALSE, LEFT_JUSTIFIED );
	usPosY += GIO_GAP_BN_SETTINGS-5;
...
void BtnBROptionTogglesCallback( GUI_BUTTON *btn, INT32 reason )
{
	if( reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		...
		if( btn->uiFlags & BUTTON_CLICKED_ON )
		{
			...
			for( cnt=0; cnt<NUM_BR_OPTIONS; cnt++)
			{
				ButtonList[ guiBROptionToggles[ cnt ] ]->uiFlags &= ~BUTTON_CLICKED_ON;
			}
		...
		else
		{
			...
			for( cnt=0; cnt<NUM_BR_OPTIONS; cnt++)
			{
				if( ButtonList[ guiBROptionToggles[ cnt ] ]->uiFlags & BUTTON_CLICKED_ON )
				{
					fAnyChecked = TRUE;
				}
			}
