...
//	if (Item[pSoldier->inv[HANDPOS].usItem].usItemClass & IC_GUN)
	{
		if (pSoldier->bDoBurst)
		{
			...
			else
			{
				if ( pSoldier->bWeaponMode == WM_ATTACHED_GL_BURST )
					...
					pSoldier->bBulletsLeft = __min( GetShotsPerBurst(&pSoldier->inv[ pSoldier->ubAttackingHand ]), pSoldier->inv[ pSoldier->ubAttackingHand ].ubGunShotsLeft );
			}
		}
		...
		{
			pSoldier->bBulletsLeft *= AmmoTypes[pSoldier->inv[ pSoldier->ubAttackingHand ].ubGunAmmoType].numberOfBullets;
		}
	}
...
BOOLEAN IsValidSecondHandShot( SOLDIERTYPE *pSoldier )
{
	if ( Item[ pSoldier->inv[ SECONDHANDPOS ].usItem ].usItemClass == IC_GUN && 
			 !(Item[ pSoldier->inv[SECONDHANDPOS ].usItem ].twohanded ) &&
			 !pSoldier->bDoBurst && 
